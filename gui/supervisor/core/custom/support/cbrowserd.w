...
detalle-items BUTTON-1 
&Scoped-Define DISPLAYED-OBJECTS c_SortBy c_Lock c_Key l_Enable c_Layout ~
l_View c_create detalle-items 
...
DEFINE VARIABLE detalle-items AS INTEGER 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "Detalle", 1,
"Items", 2
     SIZE 42 BY 1.52 NO-UNDO.
...
DEFINE FRAME Attribute-Dlg
     ...
     "  Create new record immediately on 'Add'?" VIEW-AS TEXT
          ...
          BGCOLOR 1 FGCOLOR 15 
     "tipo cabecera detalle" VIEW-AS TEXT
          SIZE 60 BY .62 AT ROW 10.29 COL 2
          BGCOLOR 1 FGCOLOR 15 
     SPACE(1.20) SKIP(1.84)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "SmartBrowser Attributes":L.
...
ON GO OF FRAME Attribute-Dlg /* SmartBrowser Attributes */
DO:     
  ...
  attr-list = "".
  if detalle-items:screen-value = "detalle" then attr-list = "tipo-detalle=detalle".
  if detalle-items:screen-value = "items" then attr-list = "tipo-detalle=items".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Attribute-Dlg Attribute-Dlg
ON WINDOW-CLOSE OF FRAME Attribute-Dlg /* SmartBrowser Attributes */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-1 Attribute-Dlg
ON CHOOSE OF BUTTON-1 IN FRAME Attribute-Dlg /* Aceptar */
DO:
...
------------------------------------------------------------------------------*/
  DISPLAY c_SortBy c_Lock c_Key l_Enable c_Layout l_View c_create detalle-items 
      WITH FRAME Attribute-Dlg.
  ENABLE c_Lock l_Enable l_View c_create detalle-items BUTTON-1 
      WITH FRAME Attribute-Dlg.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:   
    ...
    RUN get-attribute IN p_hSMO ("tipo-detalle":U).
    c_Create = RETURN-VALUE.
    if c_create = "detalle" then detalle-items:screen-value = "1" .
    else if c_create = "items" then detalle-items:screen-value = "2".
