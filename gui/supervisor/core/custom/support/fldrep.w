...
&Scoped-Define ENABLED-OBJECTS brws-keys BUTTON-1 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS MULTIPLE SIZE 87 BY 8.1.
...
DEFINE FRAME f-dlg
     ...
     b_Remove AT ROW 2.57 COL 90
     BUTTON-1 AT ROW 4.05 COL 90
     SPACE(0.19) SKIP(5.85)
...
ON CHOOSE OF b_Remove IN FRAME f-dlg /* Borrar */
DO:
  DEFINE VAR i AS INTEGER NO-UNDO.
...
run adeuib/_uibinfo.p(input procid,input procid,input "FRAMES", output cresult).
run adeuib/_uibinfo.p(input procid, input cresult ,input "context", output i_contex).

RUN adeuib/_uibinfo.p (procid,cresult, "CONTAINS FILL-IN" , OUTPUT i_contex).
...
do i=1 to num-entries(i_contex):
    ...
    do:
        RUN adeuib/_uibinfo.p (integer(entry(i,i_contex)),cresult,"handle" , OUTPUT h).  
        create tt1.
        tt1.db-field = cresult.
        tt1.tipo = h:data-type.
    end.    
end.


for each tt :
    find first tt1 where tt1.db-field = tt.db-field no-error.
    if not available tt1 then
        delete tt.
end.        

for each tt1 :
    find first tt where tt.db-field = tt1.db-field no-error.
    if not available tt then
    do:
        RUN adeuib/_uibinfo.p (procid ,tt1.db-field,"handle" , OUTPUT h).  
        if valid-handle(h) then
        do:
            create tt.
            assign tt.db-field = tt1.db-field.
        end.    
    end.
end.        
...
do i = 1 to cnt:
    ...
    c=entry(1,base).
    campo=entry(2,base).
    find first tt1 where tt1.db-field = c no-error.
    if c <> "" then
    do:
...
define var srecid as integer no-undo.
define var p_code as character no-undo.
define var nombre-tabla as character.
define var campo as character no-undo.
...
run adeuib/_accsect.p ("SET" , input procid, "PROCEDURE:habilitar_relacion", 
                       input-output srecid, input-output p_code).   
for each tt :
    if retorna_campo(tt.db-field,2) <> "" then
        campo = retorna_campo(tt.db-field,2).
    else
        campo = tt.db-field.
    if tt.program <> "" then
        if lista_relacion = "" then
            lista_relacion = campo.
        else lista_relacion = lista_relacion + "," + campo.
end.
...
for each tt :
    if retorna_campo(tt.related-db-field,1) <> "" then
        lista_relacion = lista_relacion + tt.db-field + "," + tt.related-db-field + chr(10).
end.
...
Debe tener siempre el parámetro output r" view-as alert-box error .
    ...
    p_code = "do: " + chr(10) +
        ...
        c + ":screen-value = " + "string(" + retorna_tabla(entry(3,base,";"),1) + "." + retorna_campo(c,2) + ")." + chr(10). 
...
define var cur-control as handle.
define var lista_relacion as character no-undo initial "comercial.articulos.id_proveedor".
