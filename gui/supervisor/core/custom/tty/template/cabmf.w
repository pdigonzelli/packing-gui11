...
DEFINE RECTANGLE RECT-4
     ...
     &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 142 BY 5
     &ELSE SIZE 142 BY 5 &ENDIF.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'custom/tty/objects/cus-updsav.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
                     SmartPanelType = Save,
                     AddFunction = Multiple-Records':U ,
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 2.00 , 3.00 ) NO-ERROR.
       RUN set-size IN h_cus-updsav ( 3.00 , 71.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'custom/tty/objects/cus-navico.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
                     SmartPanelType = NAV-ICON,
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 2.00 , 75.00 ) NO-ERROR.
       RUN set-size IN h_cus-navico ( 3.00 , 49.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'custom/tty/objects/cus-misc.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
                     SmartPanelType = Misc-icon':U ,
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 2.00 , 125.00 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 3.00 , 19.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'custom/tty/template/cfolder.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_cfolder ).
       RUN set-position IN h_cfolder ( 8.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_cfolder ( 18.00 , 142.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_cfolder ,
             h_cus-misc , 'AFTER':U ).
    END. /* Page 0 */
