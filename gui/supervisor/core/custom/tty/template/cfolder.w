...
&Scoped-Define ENABLED-OBJECTS Rect-Main Rect-Left Rect-Top Rect-Right ~
Rect-Bottom 
...
DEFINE RECTANGLE Rect-Left
     EDGE-PIXELS 0  
     &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 1 BY 4
     &ELSE SIZE 1 BY 4 &ENDIF.

DEFINE RECTANGLE Rect-Main
     EDGE-PIXELS 1 GRAPHIC-EDGE  
     &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 49 BY 5
     &ELSE SIZE 49 BY 5.28 &ENDIF.
...
DEFINE FRAME Folder-Frm
     Rect-Main
          ...
          &ELSE AT ROW 1.72 COL 1 &ENDIF
     Rect-Left
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 2 COL 1
          &ELSE AT ROW 2 COL 1 &ENDIF
     Rect-Top
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 2 COL 1
          &ELSE AT ROW 2 COL 1 &ENDIF
     Rect-Right
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 2 COL 34
          &ELSE AT ROW 2 COL 34 &ENDIF
     Rect-Bottom
...
ASSIGN 
       ...
       FRAME Folder-Frm:HEIGHT           = 7
       FRAME Folder-Frm:WIDTH            = 49.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE nro-paginas C-Win 
PROCEDURE nro-paginas :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
define output parameter nro-paginas as integer.
nro-paginas=number-of-pages.
END PROCEDURE.
