...
DEFINE VARIABLE h_bprueba2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_cp-updsav AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/cp-updsav.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
                     SmartPanelType = Save,
                     AddFunction = One-Record':U ,
             OUTPUT h_cp-updsav ).
       RUN set-position IN h_cp-updsav ( 1.00 , 6.00 ) NO-ERROR.
       RUN set-size IN h_cp-updsav ( 2.00 , 67.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'custom/tty/samples/bprueba2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bprueba2 ).
       RUN set-position IN h_bprueba2 ( 4.00 , 6.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.00 , 66.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'custom/tty/samples/v.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v ).
       RUN set-position IN h_v ( 17.00 , 6.00 ) NO-ERROR.
       /* Size in UIB:  ( 16.00 , 78.00 ) */

       /* Links to SmartViewer h_v. */
       RUN add-link IN adm-broker-hdl ( h_bprueba2 , 'Record':U , h_v ).
       RUN add-link IN adm-broker-hdl ( h_cp-updsav , 'TableIO':U , h_v ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bprueba2 ,
               h_cp-updsav , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v ,
               h_bprueba2 , 'AFTER':U ).
    END. /* Page 0 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE habilitar_relacion_viewer W-Win 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE salir W-Win 
PROCEDURE salir :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
