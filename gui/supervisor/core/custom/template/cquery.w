...
&SCOPED-DEFINE TABLA-PRINCIPAL ARTICULOS
&IF DEFINED(TABLA-PRINCIPAL)  &THEN
DEFINE BUFFER B-TABLA FOR {&TABLA-PRINCIPAL}.
&ENDIF
...
&Scoped-define PROCEDURE-TYPE skn-query

&Scoped-define ADM-SUPPORTED-LINKS Record-Source,Record-Target,Navigation-Target,vinculado-Source

&Scoped-define QUERY-NAME Query-Main

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES {&TABLA-PRINCIPAL}
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY {&SELF-NAME} FOR EACH {&TABLA-PRINCIPAL} WHERE                             OF B-TABLA NO-LOCK.
&Scoped-define TABLES-IN-QUERY-Query-Main {&TABLA-PRINCIPAL}
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main {&TABLA-PRINCIPAL}
...
/* Query rebuild information for QUERY Query-Main
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH {&TABLA-PRINCIPAL} WHERE
                            OF B-TABLA NO-LOCK
     _END_FREEFORM
...
DEFINE VAR R AS ROWID NO-UNDO.
  ...
  DO:
    RUN GET-ROWID1 IN HCONTAINER ( OUTPUT R ).
    FIND B-TABLA WHERE ROWID(B-TABLA) = R NO-LOCK NO-ERROR.
  END.
  /* Dispatch standard ADM method.                             */
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "{&TABLA-PRINCIPAL}"}
