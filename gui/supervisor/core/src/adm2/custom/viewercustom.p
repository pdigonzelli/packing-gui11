...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR lViewerField           AS LOGICAL      NO-UNDO.
...
DO j = 1 TO jCount:
    DO i = 1 TO iCount:
       ...
       IF hWIDGET:TYPE = "procedure" THEN NEXT.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE getFieldHandle Procedure 
...
DEFINE OUTPUT   PARAMETER hField    AS HANDLE    NO-UNDO.
...
{get EnabledHandles cEnabledFields}.



DO iField = 1 TO NUM-ENTRIES(cEnabledFields):
       
    hField = WIDGET-HANDLE(ENTRY(iField, cEnabledFields)).
    IF hField:NAME = pcField THEN
       RETURN.    
END.
