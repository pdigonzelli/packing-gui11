...
RUN postUpdate IN TARGET-PROCEDURE ( INPUT hBuffer) NO-ERROR.
RETURN RETURN-VALUE.
...
IF  hRowMod:BUFFER-VALUE = 'A' OR hRowMod:BUFFER-VALUE = 'C' THEN
    RUN preCreate IN TARGET-PROCEDURE ( INPUT phRowObjUpd) NO-ERROR.
ELSE
    RUN preUpdate IN TARGET-PROCEDURE ( INPUT hBuffer) NO-ERROR.
RETURN RETURN-VALUE.
END PROCEDURE.
